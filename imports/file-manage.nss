...
{
	menu(separator="after" title=title.copy_path image=[\uE05E])
	{
	...
	{
		item(title="All" image=\uE254 cmd=command.select_all)
	...
	separator
	menu(title="Show/Hide" image=[\uE12E])
	{
		item(title="System files" image=[\uE078] cmd='@command.togglehidden')
		item(title="File name extensions" image=[\uE0C0] cmd='@command.toggleext')
	}

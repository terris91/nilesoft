...
menu(where=@(this.count == 0) type='taskbar' image=icon.settings expanded=true)
{
	...
	item(title=title.desktop image=[\uE091] cmd=command.toggle_desktop)
	item(title=title.settings image=icon.settings(auto, image.color1) cmd='ms-settings:')
	item(title=title.task_manager sep=both image=[\uE159] cmd='taskmgr.exe')
	item(title=title.taskbar_Settings sep=both image=[\uE069] cmd='ms-settings:taskbar')
